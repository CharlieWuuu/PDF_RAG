...
切塊改為 300 字後，單塊脈絡較短，因此 top-k 由 5 調整為 8，約等於 2400 字元的參考範圍。

實測同一個問題（「臺灣的族群有哪些？各有什麼特色？」）：取 5 塊時答案 565 字，取 8 塊時 772 字，且後者納入了圖表判讀產生的統計數據（51.7% / 48.3%）——取太少會讓圖表描述落在門檻外。參數為 `TOP_K`。
